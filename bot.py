...
/done [номер] - отметить задачу как выполненной
/remind [номер] [минуты] - установить напоминание
/history [дни] - показать задачи за последние N дней
...
/history [дни]
• Показывает задачи за последние N дней
• Пример: /history 7 (задачи за последнюю неделю)
...
async def list_tasks_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not tasks:
        await update.message.reply_text("📋 У вас пока нет активных задач!", reply_markup=get_main_reply_markup())
        return
    ...
    if len(message) > 4000:
        ...
        for i in range(0, len(message), 4000):
            ...
            await update.message.reply_text(part, reply_markup=get_main_reply_markup() if i + 4000 >= len(message) else None)
    else:
