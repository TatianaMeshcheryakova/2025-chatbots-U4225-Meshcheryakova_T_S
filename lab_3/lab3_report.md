...
| **Ошибка зависимости PTB** | При запуске Webhook возникла `RuntimeError`, так как пакет `python-telegram-bot` был установлен без дополнительных компонентов для Webhook. | Исправление `requirements.txt` на **`python-telegram-bot[webhooks]==20.7`**  |
...
| **Проблема с публичным URL** | Некорректный URL для Webhook. | Получение корректного публичного URL из настроек Railway и его установка через `setWebhook`.| 
...
После устранения всех ошибок деплоя, бот успешно запустился и начал отвечать на команды.
